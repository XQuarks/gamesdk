...
#define VK_DYNAMIC_STATE_RANGE_SIZE \
...
void logEvent(TimingEvent event) {
  ...
  switch (event) {
    case EVENT_CALLING_ANI:
      DbgMsg(
          "About to call vkAcquireNextImageKHR at %llu nsec"
          " (delta %llu)",
          timing.now, delta);
      break;
    case EVENT_CALLED_ANI:
      DbgMsg(
          "After calling vkAcquireNextImageKHR at %llu nsec"
    ...
    case EVENT_CALLING_QS:
      DbgMsg(
          "About to call vkQueueSubmit at         %llu nsec"
          " (delta %llu)",
          timing.now, delta);
      break;
    case EVENT_CALLED_QS:
      DbgMsg(
          "After calling vkQueueSubmit at         %llu nsec"
          " (delta %llu)",
          timing.now, delta);
      break;
    case EVENT_CALLING_QP:
      DbgMsg(
          "About to call vkQueuePresentKHR at     %llu nsec"
          " (delta %llu)",
          timing.now, delta);
      break;
    case EVENT_CALLED_QP:
      DbgMsg(
          "After calling vkQueuePresentKHR at     %llu nsec"
          " (delta %llu)",
          timing.now, delta);
      break;
    case EVENT_CALLING_WFF:
      DbgMsg(
          "About to call vkWaitForFences at       %llu nsec"
          " (delta %llu)",
          timing.now, delta);
      break;
    case EVENT_CALLED_WFF:
      DbgMsg(
          "After calling vkWaitForFences at       %llu nsec"
          " (delta %llu)",
          timing.now, delta);
      break;
  }
...
VKAPI_ATTR VkBool32 VKAPI_CALL debug_messenger_callback(
  ...
  if (pCallbackData->cmdBufLabelCount > 0) {
    ...
    for (uint32_t cmd_buf_label = 0;
         cmd_buf_label < pCallbackData->cmdBufLabelCount; ++cmd_buf_label) {
...
bool ActualTimeLate(uint64_t desired, uint64_t actual, uint64_t rdur) {
  ...
  // occurred.  In almost every case, the actual time should be later than the
...
bool CanPresentEarlier(uint64_t earliest, uint64_t actual, uint64_t margin,
                       uint64_t rdur) {
  if (earliest < actual) {
    // Consider whether this present could have occurred earlier.  Make sure
    ...
    if ((diff >= (2 * MILLION)) && (margin >= (2 * MILLION))) {
      // This present could have occurred earlier because both: 1) the
...
void DemoUpdateTargetIPD(struct demo *demo) {
  ...
  err = demo->fpGetPastPresentationTimingGOOGLE(demo->device, demo->swapchain,
                                                &count, NULL);
  assert(!err);
  if (count) {
    ...
    for (uint32_t i = 0; i < count; i++) {
      if (!demo->syncd_with_actual_presents) {
        ...
        break;
      } else if (CanPresentEarlier(
                     past[i].earliestPresentTime, past[i].actualPresentTime,
                     past[i].presentMargin, demo->refresh_duration)) {
        ...
        if (demo->last_early_id == past[i].presentID) {
          ...
          uint32_t howManyPresents =
              (uint32_t)((lastEarlyTime - past[i].actualPresentTime) /
                         demo->target_IPD);
    ...
    if (early) {
      // Since we've seen at least two-seconds worth of presents that
      // could have occurred earlier than desired, let's decrease the
...
static void demo_draw(struct demo *demo) {
  ...
  if (demo->swappy_enabled) {
    SwappyVk_recordFrameStart(demo->present_queue, demo->swapchain,
                              demo->current_buffer);
  }
...
void swappy_trace_test_postWait(void *userData, int64_t cpu_time_ns,
                                int64_t gpu_time_ns) {
  DbgMsg("swappy_trace_test_postWait: cpu: %ld gpu: %ld", (long)cpu_time_ns,
         (long)gpu_time_ns);
}
void swappy_trace_test_preSwapBuffers(void *userData) {
  DbgMsg("swappy_trace_test_preSwapBuffers");
}
void swappy_trace_test_postSwapBuffers(void *userData,
                                       int64_t desiredPresentationTimeMillis) {
  DbgMsg("swappy_trace_test_postSwapBuffers: %ld",
         (long)desiredPresentationTimeMillis);
}
void swappy_trace_test_startFrame(void *userData, int currentFrame,
                                  int64_t desiredPresentationTimeMillis) {
  DbgMsg("swappy_trace_test_startFrame: %d, %ld", currentFrame,
         (long)desiredPresentationTimeMillis);
}
...
static void demo_prepare_buffers(struct demo *demo) {
  ...
  if (demo->VK_GOOGLE_display_timing_enabled) {
    ...
    err = demo->fpGetRefreshCycleDurationGOOGLE(demo->device, demo->swapchain,
                                                &rc_dur);
    assert(!err);
  ...
  assert(SwappyVk_initAndGetRefreshCycleDuration(
      env, demo->swappy_init_data.jactivity, demo->gpu, demo->device,
      demo->swapchain, &demo->refresh_duration));
  ...
  DbgMsg("Swappy swap interval = %" PRIu64 " ns",
         SwappyVk_getSwapIntervalNS(demo->swapchain));
...
static void demo_prepare_texture_buffer(struct demo *demo, const char *filename,
  ...
  err =
      vkCreateBuffer(demo->device, &buffer_create_info, NULL, &tex_obj->buffer);
  assert(!err);
  ...
  pass =
      memory_type_from_properties(demo, mem_reqs.memoryTypeBits, requirements,
...
static void demo_destroy_texture(struct demo *demo,
  ...
  for (i = 0; i < DEMO_TEXTURE_COUNT; i++) {
    ...
    if ((props.linearTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT) &&
      ...
      vkCmdCopyBufferToImage(
          demo->cmd, demo->staging_texture.buffer, demo->textures[i].image,
...
static void demo_cleanup(struct demo *demo) {
  ...
  // If the window is currently minimized, demo_resize has already done some
  // cleanup for us.
  if (!demo->is_minimized) {
    for (i = 0; i < demo->swapchainImageCount; i++) {
      vkDestroyFramebuffer(
          demo->device, demo->swapchain_image_resources[i].framebuffer, NULL);
    }
...
static VkResult demo_create_display_surface(struct demo *demo) {
  ...
  err =
      vkGetPhysicalDeviceDisplayPropertiesKHR(demo->gpu, &display_count, NULL);
  assert(!err);
  ...
  display_count = 1;
  err = vkGetPhysicalDeviceDisplayPropertiesKHR(demo->gpu, &display_count,
                                                &display_props);
  ...
  mode_count = 1;
  err = vkGetDisplayModePropertiesKHR(demo->gpu, display, &mode_count,
                                      &mode_props);
  assert(!err || (err == VK_INCOMPLETE));

  // Get the list of planes
  err = vkGetPhysicalDeviceDisplayPlanePropertiesKHR(demo->gpu, &plane_count,
                                                     NULL);
  assert(!err);
  ...
  for (plane_index = 0; plane_index < plane_count; plane_index++) {
    ...
    err = vkGetDisplayPlaneSupportedDisplaysKHR(demo->gpu, plane_index,
    ...
    err = vkGetDisplayPlaneSupportedDisplaysKHR(
        demo->gpu, plane_index, &supported_count, supported_displays);
    assert(!err);
  ...
  vkGetDisplayPlaneCapabilitiesKHR(demo->gpu, mode_props.displayMode,
                                   plane_index, &planeCaps);
  ...
  return vkCreateDisplayPlaneSurfaceKHR(demo->inst, &create_info, NULL,
                                        &demo->surface);
}
...
static struct SwappyVkFunctionProvider theVkFunctionProvider = {
    FunctionProviderInit, FunctionProviderGetProcAddr, FunctionProviderClose};
...
static void demo_init_vk(struct demo *demo) {
  ...
  if (instance_extension_count > 0) {
    ...
    for (uint32_t i = 0; i < instance_extension_count; i++) {
      ...
      if (!strcmp(VK_MVK_IOS_SURFACE_EXTENSION_NAME,
                  instance_extensions[i].extensionName)) {
        platformSurfaceExtFound = 1;
        demo->extension_names[demo->enabled_extension_count++] =
  ...
  if (!platformSurfaceExtFound) {
    ...
    ERR_EXIT(
        "vkEnumerateInstanceExtensionProperties failed to find "
        "the " VK_MVK_IOS_SURFACE_EXTENSION_NAME
        " extension.\n\n"
        "Do you have a compatible Vulkan installable client driver (ICD) "
        "installed?\n"
        "Please look at the Getting Started guide for additional "
    ...
    ERR_EXIT(
        "vkEnumerateInstanceExtensionProperties failed to find "
        "the " VK_MVK_MACOS_SURFACE_EXTENSION_NAME
        " extension.\n\n"
        "Do you have a compatible Vulkan installable client driver (ICD) "
        "installed?\n"
        "Please look at the Getting Started guide for additional "
  ...
  if (device_extension_count > 0) {
    ...
    SwappyVk_determineDeviceExtensions(demo->gpu, device_extension_count,
                                       device_extensions,
                                       &swappy_required_extension_count, NULL);
    swappy_required_extension_names =
        malloc(swappy_required_extension_count * sizeof(char *));
    for (uint32_t i = 0; i < swappy_required_extension_count; i++) {
      swappy_required_extension_names[i] =
          malloc(VK_MAX_EXTENSION_NAME_SIZE + 1);
    }
    SwappyVk_determineDeviceExtensions(
        demo->gpu, device_extension_count, device_extensions,
        &swappy_required_extension_count, swappy_required_extension_names);
    for (uint32_t i = 0; i < swappy_required_extension_count; i++) {
      demo->extension_names[demo->enabled_extension_count++] =
          swappy_required_extension_names[i];
      demo->extension_names_to_free[demo->extension_names_to_free_count++] =
          swappy_required_extension_names[i];
...
// Create a WSI surface for the window:
  ...
  err =
      vkCreateWaylandSurfaceKHR(demo->inst, &createInfo, NULL, &demo->surface);
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
  ...
  err =
      vkCreateAndroidSurfaceKHR(demo->inst, &createInfo, NULL, &demo->surface);
#elif defined(VK_USE_PLATFORM_XLIB_KHR)
  ...
  vkGetDeviceQueue(demo->device, demo->graphics_queue_family_index, 0,
                   &demo->graphics_queue);
  SwappyVk_setQueueFamilyIndex(demo->device, demo->graphics_queue,
  ...
  if (!demo->separate_present_queue) {
    ...
    vkGetDeviceQueue(demo->device, demo->present_queue_family_index, 0,
                     &demo->present_queue);
    SwappyVk_setQueueFamilyIndex(demo->device, demo->present_queue,
...
#elif defined(VK_USE_PLATFORM_IOS_MVK) || defined(VK_USE_PLATFORM_MACOS_MVK)
static void demo_main(struct demo *demo, void *view, int argc,
                      const char *argv[]) {
...
void update_gpu_workload(int32_t new_workload) {
  ...
  // We don't need to update the draw commands since gpu_workload is uploaded as
  // a uniform every frame.
}
