...
void ErrorCollector::AddError(const std::string& file_name, int line,
                              int column, const std::string& message) {
  std::cerr << "Error in: " << file_name << " line: " << line
            << " column: " << column << " message: " << message << std::endl;
}

#define ZERROR(CODE, MSG)          \
  {                                \
    last_error_code_ = CODE;       \
    {                              \
      std::stringstream str;       \
      str << MSG;                  \
      last_error_msg_ = str.str(); \
    }                              \
    return nullptr;                \
  }

ZipSourceTree::ZipSourceTree(
    const std::shared_ptr<android::ZipFileRO>& zip_file,
    const std::string& include_path)
    : zip_file_(zip_file), include_path_(include_path), last_error_code_(0) {}

pb::io::ZeroCopyInputStream* ZipSourceTree::Open(
  ...
  if (file_cache_.end() == it) {
    ...
    uint32_t file_len = 0;
    if (!zip_file_->getEntryInfo(entry, NULL, &file_len, NULL, NULL, NULL,
                                 NULL)) {
...
std::string ZipSourceTree::GetLastErrorMessage() { return last_error_msg_; }

int ZipSourceTree::GetLastErrorCode() { return last_error_code_; }

const std::shared_ptr<android::ZipFileRO>& ZipSourceTree::ZipFile() const {
  return zip_file_;
}

std::string ZipSourceTree::IncludePath() const { return include_path_; }
...
int ApkValidator::Validate(bool enforce_enums_in_annotations,
  ...
  if (!fdesc)
    ERROR(source_tree_.GetLastErrorCode(), source_tree_.GetLastErrorMessage());
  if (debug_) std::cout << "Uncompressed " << kDevProtoFilename << std::endl;
  DebugFileDesc(fdesc);

  // Check Annotations
  auto adesc = FindMessageIgnoringScope(fdesc, "Annotation");
  if (!adesc) ERROR(ERROR_GETTING_ANNOTATION, "Error finding Annotation");
  std::vector<int> def_ann_enum_size;
  auto aok = ValidateAnnotation(adesc, enforce_enums_in_annotations,
                                def_ann_enum_size);
  if (aok != NO_ERROR) ERROR(aok, "Error in Annotations");
  ...
  if (!tfdesc)
    ERROR(source_tree_.GetLastErrorCode(), source_tree_.GetLastErrorMessage());
  if (debug_) std::cout << "Uncompressed " << kTfProtoFilename << std::endl;
  DebugFileDesc(tfdesc);
  auto sdesc = FindMessageIgnoringScope(tfdesc, "Settings");
  if (!sdesc) ERROR(ERROR_GETTING_SETTINGS, "Error finding Settings");
  auto sok = ValidateSettings(sdesc, def_ann_enum_size);
  if (sok != NO_ERROR) ERROR(sok, "Error validating settings");
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (field->type() == pb::FieldDescriptor::TYPE_ENUM) {
      ...
      if (enforce_enums_in_annotations) {
        ERROR(ERROR_ANNOTATION_ONLY_ENUMS, "Annotation can only contain enums");
      } else {
...
int ApkValidator::ValidateSettings(const pb::Descriptor* sdesc,
  ...
  if (!m) ERROR(ERROR_GETTING_SETTINGS, "Error making Settings object");
  ...
  if (debug_) std::cout << "Uncompressed " << kSettingsFilename << std::endl;
  const void* ptr = 0;
  int sz = 0;
  ...
  auto aggregation_strategy_field =
      m->GetDescriptor()->FindFieldByName("aggregation_strategy");
  const pb::Message& agg_strat =
      refl->GetMessage(*m, aggregation_strategy_field);
  const pb::Reflection* as_refl = agg_strat.GetReflection();
  auto max_ikeys_field =
      agg_strat.GetDescriptor()->FindFieldByName("max_instrumentation_keys");
  ...
  auto ann_enum_size_field =
      agg_strat.GetDescriptor()->FindFieldByName("annotation_enum_size");
  std::vector<int> ann_enum_size;
  int n = as_refl->FieldSize(agg_strat, ann_enum_size_field);
  for (int i = 0; i < n; ++i)
    ann_enum_size.push_back(
        as_refl->GetRepeatedInt32(agg_strat, ann_enum_size_field, i));
  if (debug_) {
    std::cout << "Deduced annotation enum sizes from Annotation: "
              << def_ann_enum_size << std::endl;
    std::cout << "Annotation enum sizes in settings file: " << ann_enum_size
              << std::endl;
  }
  if (ann_enum_size.size() != def_ann_enum_size.size())
    ERROR(ERROR_BAD_ANNOTATION_ENUM_SIZE,
          "Annotation enum size length bad: " << ann_enum_size.size() << "!="
                                              << def_ann_enum_size.size());
  for (int i = 0; i < ann_enum_size.size(); ++i) {
    if (def_ann_enum_size[i] != -1 && ann_enum_size[i] != def_ann_enum_size[i])
      ERROR(ERROR_BAD_ANNOTATION_ENUM_SIZE,
            "Bad annotation enum size: " << ann_enum_size[i]
                                         << "!=" << def_ann_enum_size[i]);
  }
  return NO_ERROR;
}
...
int ApkValidator::ValidateDevFidelityParams() {
  ...
  std::string prefix =
      source_tree_.IncludePath() + "dev_tuningfork_fidelityparams_";
  char suffix[] = "bin";
  zip_file->startIteration(&cookie, prefix.c_str(), suffix);
  int fp_count = 0;
  for (auto entry = zip_file->nextEntry(cookie); entry;
       entry = zip_file->nextEntry(cookie)) {
    ++fp_count;
  }
  zip_file->endIteration(cookie);
  if (debug_)
    std::cout << "Found " << fp_count << " files matching " << prefix + "*.bin"
              << std::endl;
  if (fp_count == 0)
...
const pb::Descriptor* ApkValidator::FindMessageIgnoringScope(
...
void ApkValidator::DebugFileDesc(const pb::FileDescriptor* fdesc) {
  if (debug_) {
    std::cout << fdesc->name() << ": messages: " << fdesc->message_type_count()
              << ", dependencies: " << fdesc->dependency_count()
              << ", extensions: " << fdesc->extension_count()
              << ", enums: " << fdesc->enum_type_count() << std::endl;
  }
}

void ApkValidator::DebugDesc(const pb::Descriptor* desc) {
  if (debug_) {
    std::cout << desc->name() << ": fields: " << desc->field_count()
